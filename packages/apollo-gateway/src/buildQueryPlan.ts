...
function getInternalFragment(fields: Field<GraphQLCompositeType>[], returnType: GraphQLCompositeType, context: QueryPlanningContext) {
  const selectionSet = selectionSetFromFieldSet(fields, returnType);

  const key = print(selectionSet);
  let name: string;
  let definition: FragmentDefinitionNode;
  let selection: SelectionSetNode;
  if (context.internalFragments[key]) {
    ({ name, definition, selection } = context.internalFragments[key]);
  }
  else {
    name = `__QueryPlanFragment_${queryPlanInternalFragmentCount++}__`;
    definition = {
    ...
    context.internalFragments[key] = { name, definition, selection };
  }
  return { name, definition, selection };
}
